...
appRoot.directive('myCarousel', function () {
  return {
    ...
    link: function ($scope, $element, $attrs) {
      ...
      var i;
      for (i = 0, len = gitImage.length; i < len; i++) {
        dots[i] = carouselindicators.children[i];
        // angular.element(carouselindicators).animate({ class: "active" });
      }

      //scrollLeft & scrollRight Functions based on event
      function scrollRight() {
        if (carouselContainer.scrollLeft >= (imageUrls.length)) {
